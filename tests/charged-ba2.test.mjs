...
test('the Sphere Rush data: 1050 dash, a contact that only binds, +1 every 0.5 s, a 15 blast, 2.0 s delay, a 5 s cooldown, and normal BA2 is unchanged', () => {
  ...
  // The sphere on the target grows dramatically, from its own size to three
  // times it by the blast, never shrinking.
  assert.deepEqual(TECH.sphereGrowth, { startScale: 1, endScale: 3 });
  ...
  assert.deepEqual([TECH.firstHit.baseLaunch, TECH.firstHit.directionalLaunch], [0, null], 'the setup never launches');
  ...
  for (const key of ['baseKnockback', 'knockbackGrowth', 'accumulatedKnockbackAxis']) {
    assert.equal(key in TECH.explosionHit, false, `no ${key}`);
  }
  const hits = [TECH.firstHit, TECH.tickHit, TECH.explosionHit];
  assert.deepEqual(hits.filter((h) => h.baseLaunch > 0), [TECH.explosionHit], 'the only launching hit');
...
test('a hit: exactly one contact that adds nothing, its first tick on the same step, the rush stops, and the target is bound and still with the sphere on it', () => {
  const d = hitDuel();
  const t = hitConfirm(d);
  assert.equal(d.events.length, 2, 'the contact, then its tick');
  ...
  assert.equal(d.events.length, 2, 'the contact happens exactly once, and no tick before 0.5 s');
  assert.equal(d.attacker.body.x, x);
...
test('the sphere on the target grows steadily through the rasen8 hold, from startScale to endScale at the blast, centred on the target', () => {
  ...
  // Steady: the same increment every step, never a shrink, a jump or a pulse.
  ...
  for (let i = 1; i < hold.length; i++) {
    assert.ok(hold[i].scale > hold[i - 1].scale, `growing at step ${i}`);
  ...
  assert.ok(close(mid, (startScale + endScale) / 2, step), 'about halfway grown halfway through the hold');
  assert.ok(close(late, endScale - step, 1e-9), 'one step short of full size just before the blast');
  assert.ok(mid > 1.9 && late > 2.9, `much bigger, not a subtle swell: ${mid} halfway, ${late} at the end`);
...
test('the growth is visual only: a sphere growing to 3x reaches, hits, binds, ticks, blasts and launches exactly like one that never grows', () => {
  const { startScale, endScale } = TECH.sphereGrowth;
  assert.ok(endScale >= 2.75, `the dramatic growth is what is under test: ${endScale}`);
  const flatDef = { ...def, chargedTechniques: { rasenRush: { ...TECH, sphereGrowth: { startScale, endScale: startScale } } } };
  const flat = makeFighter({ character: flatDef }).fighter.techniqueDefs.rasenRush;
  assert.deepEqual({ ...flat.sphereGrowth }, { startScale, endScale: startScale });
  ...
  for (const gap of [140, edge - 10, edge]) {
    ...
    if (gap < edge) {
      ...
      assert.ok(Math.max(...grown.scales.filter((v) => v !== null)) === endScale, 'this one did grow');
      assert.ok(plain.scales.every((v) => v === null || v === startScale), 'that one never did');
    } else {
...
test('the explosion comes exactly 2.0 s after the hit step on rasen9: prasen10 -> prasen11, +15, target released then launched sideways at 3 x 19; only then rasen10-12', () => {
  ...
  for (let i = 1; i < DELAY_STEPS; i++) {
    ...
    assert.equal(d.target.combat.launchPoint, 1 + Math.floor(i / TICK_STEPS), `step ${i}`);
    assert.equal(d.target.combat.immobilized, true);
  }
...
test('while bound: one 1-damage tick on the contact step itself, then one every 0.5 s of fixed steps, none early, and never on the explosion\'s step', () => {
  const d = hitDuel();
  const t = hitConfirm(d);
  ...
  assert.equal(t.ticksDue, 0, 'dealt, none left over for a second one on this step');
  const { ticks, explosions } = tickSteps(d, t);
  assert.deepEqual(ticks, [TICK_STEPS, 2 * TICK_STEPS, 3 * TICK_STEPS], 'then 0.5, 1.0 and 1.5 s after the hit step');
...
test('ticks begin only once the rush has caught someone: a whiff, and the rush before contact, tick nothing', () => {
  ...
  assert.equal(miss.target.combat.launchPoint, 0);
  const hit = hitDuel();
  const u = start(hit);
...
test('a tick adds 1 Launch Point and nothing else: no launch, no stun or freeze, the hold never stutters', () => {
  const d = hitDuel();
  const t = hitConfirm(d);
  ...
  // Even at a huge Launch Point, a tick never launches: 0 x 120 and 0 x 901
  // are both 0.
  // (The contact's tick, then the one 0.5 s later.)
  for (const [from, to] of [[119, 121], [900, 902]]) {
    const big = hitDuel();
    big.target.combat.launchPoint = from;
    hitConfirm(big);
    for (let i = 0; i < TICK_STEPS; i++) big.tick();
    assert.equal(big.target.combat.launchPoint, to);
    assert.equal(big.events.at(-1).launchStrength, 0);
...
test('the blast adds its 15 first, then launches at exactly 3 x the new Launch Point, sideways: 106 + 15 = 121, 363', () => {
  ...
  const blastAt = (from, facing = 1) => {
    ...
    d.target.combat.launchPoint = from;
    const t = hitConfirm(d);
...
test('an interruption stops the ticks: a hit on #0001 after the 0.5 s tick leaves exactly the two so far', () => {
  const d = hitDuel();
  const t = hitConfirm(d);
  ...
  assert.equal(d.events.filter((e) => e.technique).length, 3, 'the contact and the two ticks');
});
...
test('a lost bind stops the ticks: nothing more once the target is no longer held', () => {
  ...
  e.tick();
  assert.equal(e.target.combat.launchPoint, 1, 'the contact\'s tick only');
...
test('the Void stops the ticks: a caught target lost to it takes nothing more', async () => {
  ...
  assert.equal(battle.p2.combat.launchPoint, 2, 'the contact\'s tick and the 0.5 s one');
  ...
  assert.equal(t.endReason, 'released');
  // Out of play for its respawn wait, keeping the Launch Point it fell with:
  // no tick or explosion reaches it.
  while (battle.p2.lostToVoid) {
    assert.equal(battle.p2.combat.launchPoint, 2);
    battle.update(DT);
  }
...
test('no Launch Point ends the hold: a target at 999 is caught, ticked and blown up like any other', () => {
  const d = hitDuel();
  d.target.combat.launchPoint = 999;
  const t = start(d);
  d.until(() => !d.attacker.technique);
  assert.equal(t.endReason, 'done');
  assert.deepEqual(d.events.map((e) => [e.type, e.damage]), FULL_RUSH);
...
test('ground lost after the hit cancels the rest: the tick dealt stays, the target is freed at once, no more ticks or explosion, #0001 falls', () => {
  ...
  const t = hitConfirm(d);
  assert.equal(d.target.combat.launchPoint, 1, 'the contact\'s tick');
  for (let i = 0; i < 40; i++) d.tick();
  assert.equal(d.target.combat.launchPoint, 2, 'two ticks so far');
  assert.equal(t.phase, 'wait');
  stage.platforms.length = 0;
  d.tick();
  assert.equal(d.attacker.technique, null);
  ...
  assert.equal(d.attacker.state, 'fall');
  for (let i = 0; i < DELAY_STEPS + 30; i++) d.tick();
  assert.equal(d.events.length, 3, 'the contact and two ticks: the rest never comes');
  assert.equal(d.target.combat.launchPoint, 2, 'what was dealt is kept');
});
...
test('a Shield blocks the contact: 25 Energy, no Launch Point, no bind, no tick, no explosion, and the rush ends cleanly', () => {
  ...
  assert.equal(d.target.combat.launchPoint, 0);
  assert.equal(d.target.combat.immobilized, false, 'never bound through a Shield');
...
test('Battle draws the sphere over both fighters, in the hand then on the target, never mirrored; debug outlines it', async () => {
  ...
  assert.equal(widths[0], base, 'caught at its own size');
  assert.ok(widths.at(-1) > base * 2.5, `grown dramatically: ${base} -> ${widths.at(-1)} px`);
