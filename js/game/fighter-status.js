...
//
//   [ stamina bar ]     purple, gray while exhausted
//   [ P1 / CPU tag ]    (Arena.drawMarkers)
//   [ fighter      ]
//   [ CAB1   CAB2  ]    charged-action cooldown rings under the feet
//
// The state helpers (staminaBarState, cabIndicators) are pure, so what is
// drawn can be checked without a canvas; the draw functions only paint it.
// Nothing here is character-specific: the rings come from the character's
// own `chargedActions`, the bar from its stamina.

// Stamina bar: a thin purple fill on a dark track with a black outline; the
// fill turns gray while the fighter is exhausted, until it is full again.
export const STAMINA_STYLE = Object.freeze({
  fill: '#a855f7',
  exhausted: '#8c8c8c',
...
// What the stamina bar shows for `fighter`: the fill ratio (stamina /
// maxStamina, 0 to 1), its colour and whether the fighter is exhausted.
export function staminaBarState(fighter) {
  const c = fighter.combat;
  const exhausted = c.staminaExhausted;
  return { ratio: c.staminaRatio, exhausted, color: exhausted ? STAMINA_STYLE.exhausted : STAMINA_STYLE.fill };
}

// One entry per charged action of `fighter`'s character, in its
// `chargedActions` order: { id, action, label, progress, ready, text }.
// progress = 1 - remaining / duration, read straight from the fighter's
// cooldowns, so a Charge that speeds recovery speeds the ring too. Ready:
// complete, and no number.
export function cabIndicators(fighter) {
  const cooldowns = fighter.combat.chargedCooldowns;
  return Object.entries(fighter.def.chargedActions ?? {}).map(([action, charged]) => {
    const remaining = cooldowns.remaining(charged.id);
    const ready = remaining <= 0;
    return {
      id: charged.id,
      action,
      label: CHARGED_LABELS[action] ?? action.toUpperCase(),
      progress: ready ? 1 : cooldowns.progress(charged.id),
      ready,
      text: ready ? '' : formatCooldown(remaining),
    };
  });
}
...
// (the outline is one CSS pixel).
export function drawStaminaBar(ctx, fighter, rect, dpr = 1) {
  const { x, y, w, h } = rect;
  const { ratio, color } = staminaBarState(fighter);
  const o = Math.max(1, Math.round(dpr));
  ctx.save();
  ctx.fillStyle = STAMINA_STYLE.outline;
  ctx.fillRect(x - o, y - o, w + o * 2, h + o * 2);
  ctx.fillStyle = STAMINA_STYLE.track;
...
// One ring per charged action in a row under the feet at (x, footY), device
// pixels: the ring completes clockwise from the top as the ability
// recovers, the seconds left inside it while cooling down, its CAB name
// beneath. White, outlined in black. Sized with the world (`scale`, device
// pixels per world unit) but never below a readable size in CSS pixels
// (`dpr`, device pixels per CSS pixel), the number always inside its ring.
