...
//
//   [ stamina bar ]     only while below full; purple, gray while exhausted
//   [ P1 / CPU tag ]    (Arena.drawMarkers)
//   [ fighter      ]
//   [ CAB1   CAB2  ]    only the charged actions cooling down, under the feet
//
// Both are temporary: a full bar and a ready charged action draw nothing, so
// a fighter with full stamina and nothing cooling down carries only its tag.
...
// fill turns gray once the fighter is exhausted and stays gray until it is
// full again, when the bar goes.
...
// What the stamina bar shows for `fighter`: whether it shows at all (only
// while stamina is below full: CombatState.setStamina snaps a refill that
// reaches the maximum to exactly it), the fill ratio (stamina / maxStamina,
// 0 to 1), its colour and whether the fighter is exhausted. Exhaustion only
// ever clears at full, so the bar is gray from 0 until the step it goes.
export function staminaBarState(fighter) {
  const c = fighter.combat;
  const exhausted = c.staminaExhausted;
  return {
    visible: c.stamina < c.maxStamina,
    ratio: c.staminaRatio,
    exhausted,
    color: exhausted ? STAMINA_STYLE.exhausted : STAMINA_STYLE.fill,
  };
}
...
// (the outline is one CSS pixel). Nothing at full stamina. True when drawn.
export function drawStaminaBar(ctx, fighter, rect, dpr = 1) {
  const { x, y, w, h } = rect;
  const { visible, ratio, color } = staminaBarState(fighter);
